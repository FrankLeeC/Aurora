package test

import (
	"testing"

	"github.com/FrankLeeC/Aurora/tools/encrypt"
)

const s = "hello 世界"

func TestPKCS1FromFile(t *testing.T) {
	pubKey, err := encrypt.GeneratePublicKeyFromFile("./pkcs1/rsa_public.pem")
	if err != nil {
		t.Errorf("generate public key from file error: %s\n", err.Error())
		return
	}
	priKey, err := encrypt.GeneratePrivateKeyFromFile("./pkcs1/rsa_private.pem", encrypt.PKCS1)
	if err != nil {
		t.Errorf("generate private key from file error: %s\n", err.Error())
		return
	}
	cipherBytes, err := encrypt.RSAEncrypt(s, pubKey)
	if err != nil {
		t.Errorf("encrypt error: %s\n", err.Error())
		return
	}
	plainBytes, err := encrypt.RSADecrypt(string(cipherBytes), priKey)
	if err != nil {
		t.Errorf("decrypt error: %s\n", err.Error())
		return
	}
	t.Log(string(plainBytes))
	if string(plainBytes) == s {
		t.Log("pass!")
	} else {
		t.Log("fail!")
	}

}

func TestPKCS1FromString(t *testing.T) {
	pubKey, err := encrypt.GeneratePublicKey([]byte(`
-----BEGIN PUBLIC KEY-----
MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCwHsMGYrbgR1XgkH0AsrZQyAhZ
oXluYNFpZfC6TE33ykJrNge+9LwP9Hs0tO7FcY52iGgxZ0Fa8JcSfRzBlJnJz14k
KGlpLEcnPBhV3W77s4RvM+ZKSEv57Z2HPRgXgJue/mcKI+DzjYGpfhMKa5+JG2Zm
Sw6yQZ7BFW4S9vGZDwIDAQAB
-----END PUBLIC KEY-----		
`))
	if err != nil {
		t.Errorf("generate public key error: %s\n", err.Error())
		return
	}
	priKey, err := encrypt.GeneratePrivateKey([]byte(`
-----BEGIN RSA PRIVATE KEY-----

fRiCujrulI4c+s0Vk6NpW3TFkhTvkx+P6j9MK0MRfDVH
-----END RSA PRIVATE KEY-----		
		`), encrypt.PKCS1)
	if err != nil {
		t.Errorf("generate private key error: %s\n", err.Error())
		return
	}
	cipherText, err := encrypt.RSAEncrypt(s, pubKey)
	if err != nil {
		t.Errorf("encrypt error: %s\n", err.Error())
		return
	}

	plainBytes, err := encrypt.RSADecrypt(string(cipherText), priKey)
	if err != nil {
		t.Errorf("decrypt error: %s\n", err.Error())
		return
	}
	t.Log(string(plainBytes))
	if string(plainBytes) == s {
		t.Log("pass!")
	} else {
		t.Log("fail!")
	}
}

func TestPKCS8FromFile(t *testing.T) {
	pubKey, err := encrypt.GeneratePublicKeyFromFile("./pkcs8/rsa_public.pem")
	if err != nil {
		t.Errorf("generate public key from file error: %s\n", err.Error())
		return
	}
	priKey, err := encrypt.GeneratePrivateKeyFromFile("./pkcs8/rsa_private_pkcs8.pem", encrypt.PKCS8)
	if err != nil {
		t.Errorf("generate private key from file error: %s\n", err.Error())
		return
	}
	cipherBytes, err := encrypt.RSAEncrypt(s, pubKey)
	if err != nil {
		t.Errorf("encrypt error: %s\n", err.Error())
		return
	}
	plainBytes, err := encrypt.RSADecrypt(string(cipherBytes), priKey)
	if err != nil {
		t.Errorf("decrypt error: %s\n", err.Error())
		return
	}
	t.Log(string(plainBytes))
	if string(plainBytes) == s {
		t.Log("pass!")
	} else {
		t.Log("fail!")
	}
}

func TestPKCS8FromString(t *testing.T) {
	pubKey, err := encrypt.GeneratePublicKey([]byte(`
-----BEGIN PUBLIC KEY-----
MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCwHsMGYrbgR1XgkH0AsrZQyAhZ
oXluYNFpZfC6TE33ykJrNge+9LwP9Hs0tO7FcY52iGgxZ0Fa8JcSfRzBlJnJz14k
KGlpLEcnPBhV3W77s4RvM+ZKSEv57Z2HPRgXgJue/mcKI+DzjYGpfhMKa5+JG2Zm
Sw6yQZ7BFW4S9vGZDwIDAQAB
-----END PUBLIC KEY-----		
		`))
	if err != nil {
		t.Errorf("generate public key error: %s\n", err.Error())
		return
	}
	priKey, err := encrypt.GeneratePrivateKey([]byte(`
-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----			
				`), encrypt.PKCS8)
	if err != nil {
		t.Errorf("generate private key error: %s\n", err.Error())
		return
	}
	cipherText, err := encrypt.RSAEncrypt(s, pubKey)
	if err != nil {
		t.Errorf("encrypt error: %s\n", err.Error())
		return
	}

	plainBytes, err := encrypt.RSADecrypt(string(cipherText), priKey)
	if err != nil {
		t.Errorf("decrypt error: %s\n", err.Error())
		return
	}
	t.Log(string(plainBytes))
	if string(plainBytes) == s {
		t.Log("pass!")
	} else {
		t.Log("fail!")
	}
}
